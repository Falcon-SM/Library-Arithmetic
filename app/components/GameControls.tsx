...
export const GameControls: React.FC = () => {
    const { rollDice, endTurn, log, turn, tileToPlace, drawBoardTile, currentRoll, highlightedTiles } = useGameStore();

    const handleRoll = () => {
        const result = rollDice();
        // In a real implementation, we'd show the result and trigger movement state
        // alert(`Rolled a ${ result } !`); // Removed alert, using log
    };
    ...
    return (
        <div className="fixed top-0 right-0 left-0 md:left-auto md:top-4 md:right-4 flex flex-col gap-2 md:gap-4 items-end z-40 pointer-events-none p-2 md:p-0">
            ...
            <div className="bg-white/95 p-2 md:p-4 rounded-xl shadow-2xl border-2 border-[#c2b280] pointer-events-auto animate-in slide-in-from-right flex items-center gap-3 self-end">
                <div className="text-xs md:text-sm uppercase tracking-widest text-gray-500 font-bold hidden md:block">Current Turn</div>
                <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <span className="text-lg md:text-2xl font-black text-[#2c1810]">
            ...
            <div className="bg-white/90 p-3 md:p-4 rounded-lg shadow-lg pointer-events-auto w-full md:w-64 max-w-sm self-end">
                <div className="flex justify-between items-center mb-2 border-b pb-2">
                    <span className="font-bold text-base md:text-lg">Turn {turn} / 8</span>
                </div>

                <div className="flex gap-2 mb-2 md:mb-4">
                    <button
                        onClick={handleRoll}
                        disabled={!!currentRoll || !!tileToPlace}
                ...
                {/* Draw Tile Button (if stuck) */}
                {currentRoll && highlightedTiles.length === 0 && !tileToPlace && (
                    <button
                        onClick={drawBoardTile}
                        className="w-full mb-2 md:mb-4 bg-amber-600 text-white py-2 rounded hover:bg-amber-700 flex items-center justify-center gap-2 font-bold shadow animate-pulse text-sm md:text-base"
                    >
                        Draw Tile
                    </button>
                )}
